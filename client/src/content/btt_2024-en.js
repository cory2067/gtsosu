export default {
  name: "Beginner's Taiko Tournament Season 9",
  description:
  ...
  links: [
    ...
    {
      label: "Challonge",
      link: "https://challonge.com/BTTS9"
    },
  ],
  homepage: [
    {
...
July 20th until July 21st.
### Grand Finals + Loser's Bracket Grand Finals + All-Stars
July 27th until July 28th.`,
    },
    {
      title: "Prizes",
      body: `
**1st place:** Special Discord role in the BTT server.
There are no other prizes being considered for BTTS9
`,
    },
  ],
  rules: `
...
- The winning conditions for each stage are as follows:
...
- If a player is not responding to a reschedule request, the hosts (TaikoMom or Astraea) should be made aware of this so they can look into this on time.
- If a player is not cooperating on a reschedule and there are no given reasons as to why, **the host may decide to make it a wbd for the other player**. This is done on a case-by-case basis and can only be decided by a host.
- If there is **hard evidence** of a player denying a reschedule request (screenshots and such), **a host may immediately kick the player from the tournament and permanently ban them from ever participating again**. This is, again, done on a case-by-case basis and can only be decided by a host.
...
- The match will be played with HeadToHead and ScoreV2. The name of the lobby must be: "BTTS9: (Red Player) vs (Blue Player)
...
- Players can at any time ask for a timeout of max 10 minutes. Each player can do so only once per match.
- If a player disconnects between the beatmaps, the match can be delayed 10 minutes at maximum. This delay can only be applied once per player per match, but is counted separately from a timeout.
